...
// Lazy execution setup for Cloudinary SDK
...
export const CloudinaryEngine = {
  ...
  /**
   * Upload an image to Cloudinary. Supports base64 data URLs or standard URIs.
   * If credentials are NOT configured, it safe-falls back to a beautiful placeholder image
   * so the system remains 100% operational in dev preview mode!
   */
  async uploadImage(fileDataUri: string): Promise<string> {
    ...
    if (!client) {
      ...
      // Return a nice fallback image based on the file type or an elegant Unsplash random design graphic
    ...
    try {
      // Cloudinary natively supports uploading base64 data URIs directly
