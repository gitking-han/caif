...
export const StorageEngine = {
  ...
  async addService(item: Partial<Service>): Promise<Service> {
    const client = await getMongoClient();
  ...
  async addPortfolio(item: Partial<Portfolio>): Promise<Portfolio> {
    const client = await getMongoClient();
  ...
  async updatePortfolio(id: string, updates: Partial<Portfolio>): Promise<Portfolio | null> {
    ...
    if (idx !== -1) {
      local.portfolio[idx] = {
        ...
        imageUrl: updates.imageUrl ?? local.portfolio[idx].imageUrl,
  ...
  async addPopup(item: Partial<Popup>): Promise<Popup> {
    const client = await getMongoClient();
  ...
  async addBlog(item: Partial<Blog>): Promise<Blog> {
    const client = await getMongoClient();
  ...
  async addContact(item: Partial<Contact>): Promise<Contact> {
    const client = await getMongoClient();
    const newContact: Contact = {
      ...
      message: item.message || "",
      intent: item.intent || "Get Started",
    ...
    if (client) {
      try {
        ...
        console.error("Mongo addContact failed:", e);
      }
    }
